...
#define MASTER_RX_BUF_SIZE  32      // Max size in bytes of message that MASTER can receive
#define MASTER_TX_BUF_SIZE  49      // Max size in bytes of message that MASTER can send
#define NUM_STATIONS_ADDR   0       // EEPROM address where master saves the # of stations
#define RNG_SEED_ADDR       1       // EEPROM address where master & station saves RNG seed
...
#define SHARED_KEY_ADDR     82      // EEPROM address where station saves its shared key
...
private:
	...
	SerialInterface serialInterface;	// Manages interface by Serial Port with PC
	...
	void sendP2P();										// Master sends to Station some data & a challenge 
...
private:
	...
	void calculateHMAC();							// Calculates HMAC of data received & station public key
