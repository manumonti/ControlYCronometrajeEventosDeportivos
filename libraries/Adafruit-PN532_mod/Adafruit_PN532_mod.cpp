...
// TFG: No esta definido la arquitectura del Genuino 101, por lo que usa 
// Wire1, que no existe en esa plataforma
#include <Wire.h>
#define WIRE Wire
...
bool Adafruit_PN532::waitready(uint16_t timeout) {
  ...
  while(!isready()) {
    if (timeout != 0) {
      timer += 10;
      if (timer > timeout) {
      	// TFG: No queremos que se muestre TIMEOUT cada vez
